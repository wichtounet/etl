...
/*!
 * \file
 * \brief Contains diagonal matrix reference proxy implementation.
 */
...
/*!
 * \brief A proxy representing a reference to a mutable element of a diagonal matrix
 * \tparam M The matrix type
 */
template <typename M>
struct diagonal_reference {
    ...
    using expr_t                   = M;                                ///< The diagonal matrix
    ...
    /*!
     * \brief Constructs a new diagonal_reference
     * \param matrix The source matrix
     * \param i The index i of the first dimension
     * \param j The index j of the second dimension
     */
    diagonal_reference(matrix_type& matrix, size_t i, size_t j) : matrix(matrix), i(i), j(j), value(matrix(i, j)) {
     ...
     */
    diagonal_reference& operator=(const value_type& rhs) {
     ...
     * \return a reference to the proxy reference
     */
    diagonal_reference& operator+=(value_type rhs) {
        if (i == j || rhs == value_type(0)) {
            value += rhs;
        } else {
            throw diagonal_exception();
...
} //end of namespace diagonal_detail

...
/*!
 * \file
 * \brief Contains strictly lower triangular matrix reference proxy implementation.
 */
...
namespace strictly_lower_detail {

/*!
 * \brief A proxy representing a reference to a mutable element of a strictly lower triangular matrix
 * \tparam M The matrix type
 */
template <typename M>
struct strictly_lower_reference {
    ...
    using expr_t                   = M;                                ///< The strictly lower triangular matrix
    ...
    /*!
     * \brief Constructs a new strictly_lower_reference
     * \param matrix The source matrix
     * \param i The index i of the first dimension
     * \param j The index j of the second dimension
     */
    strictly_lower_reference(matrix_type& matrix, size_t i, size_t j) : matrix(matrix), i(i), j(j), value(matrix(i, j)) {
     ...
     */
    strictly_lower_reference& operator=(const value_type& rhs) {
     ...
     * \return a reference to the proxy reference
     */
    strictly_lower_reference& operator+=(value_type rhs) {
        if (i > j || rhs == value_type(0)) {
            value += rhs;
        } else {
            throw strictly_lower_exception();
...
} //end of namespace strictly_lower_detail

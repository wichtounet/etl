...
struct strictly_lower_matrix final : comparable<strictly_lower_matrix<Matrix>>, iterable<const strictly_lower_matrix<Matrix>> {
...
public:
     ...
     */
    strictly_lower_matrix() noexcept : matrix(value_type()) {
     ...
     * This constructor can only be used when the matrix is fast
     */
    strictly_lower_matrix(value_type value) noexcept : matrix(value) {
        //Nothing else to init
    }
     ...
     */
    strictly_lower_matrix(std::size_t dim) noexcept : matrix(dim, dim, value_type()) {
        //Nothing else to init
    }
     ...
     * the strictly lower triangular guarantee.
     */

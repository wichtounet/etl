...
struct diagonal_matrix final : comparable<diagonal_matrix<Matrix>>, iterable<const diagonal_matrix<Matrix>> {
...
public:
     ...
     */
    diagonal_matrix() noexcept : matrix(value_type()) {
     ...
     * This constructor can only be used when the matrix is fast
     */
    diagonal_matrix(value_type value) noexcept : matrix(value) {
        //Nothing else to init
    }
     ...
     */
    diagonal_matrix(std::size_t dim) noexcept : matrix(dim, dim, value_type()) {
        //Nothing else to init
    }

...
/*!
 * \file
 * \brief Contains uni upper triangular matrix reference proxy implementation.
 */
...
namespace uni_upper_detail {

/*!
 * \brief A proxy representing a reference to a mutable element of a uni upper triangular matrix
 * \tparam M The matrix type
 */
template <typename M>
struct uni_upper_reference {
    ...
    using expr_t                   = M;                                ///< The uni upper triangular matrix
    ...
    /*!
     * \brief Constructs a new uni_upper_reference
     * \param matrix The source matrix
     * \param i The index i of the first dimension
     * \param j The index j of the second dimension
     */
    uni_upper_reference(matrix_type& matrix, size_t i, size_t j) : matrix(matrix), i(i), j(j), value(matrix(i, j)) {
    ...
    uni_upper_reference& operator%=(value_type rhs) {
        if (i == j && rhs == value_type(0)) {
            value %= rhs;
        } else if (i < j || (value % rhs) == value_type(0)) {
            value %= rhs;
        } else {
            throw uni_upper_exception();
        }
...
} //end of namespace uni_upper_detail

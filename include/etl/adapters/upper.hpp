...
struct upper_matrix final : comparable<upper_matrix<Matrix>>, iterable<const upper_matrix<Matrix>> {
...
public:
     ...
     */
    upper_matrix() noexcept : matrix(value_type()) {
     ...
     * This constructor can only be used when the matrix is fast
     */
    upper_matrix(value_type value) noexcept : matrix(value) {
        //Nothing else to init
    }
     ...
     */
    upper_matrix(std::size_t dim) noexcept : matrix(dim, dim, value_type()) {
        //Nothing else to init
    }
     ...
     * \param rhs The right-hand-side matrix
     */
    upper_matrix(upper_matrix&& rhs) = default;
     ...
     * \return a reference to the assigned matrix
     */
    upper_matrix& operator=(upper_matrix&& rhs) = default;
     ...
     */
    upper_detail::upper_reference<matrix_t> operator()(std::size_t i, std::size_t j) noexcept {
        return {matrix, i, j};
    }
     ...
     * the upper triangular guarantee.
     */

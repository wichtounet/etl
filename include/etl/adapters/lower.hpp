...
struct lower_matrix final : adapter<Matrix>, comparable<lower_matrix<Matrix>>, iterable<const lower_matrix<Matrix>> {
...
public:
     ...
     * This constructor can only be used when the matrix is fast
     */
    lower_matrix(value_type value) noexcept : base_type(value) {
        //Nothing else to init
    }
     ...
     */
    lower_matrix(std::size_t dim) noexcept : base_type(dim) {
        //Nothing else to init
    }

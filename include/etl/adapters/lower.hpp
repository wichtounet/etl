...
struct lower_matrix final : comparable<lower_matrix<Matrix>>, iterable<const lower_matrix<Matrix>> {
...
public:
     ...
     */
    lower_matrix() noexcept : matrix(value_type()) {
     ...
     * This constructor can only be used when the matrix is fast
     */
    lower_matrix(value_type value) noexcept : matrix(value) {
        //Nothing else to init
    }
     ...
     */
    lower_matrix(std::size_t dim) noexcept : matrix(dim, dim, value_type()) {
        //Nothing else to init
    }
     ...
     * the lower triangular guarantee.
     */

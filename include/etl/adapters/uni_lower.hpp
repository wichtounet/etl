...
struct uni_lower_matrix final : adapter<Matrix>, comparable<uni_lower_matrix<Matrix>>, iterable<const uni_lower_matrix<Matrix>> {
...
public:
     ...
     */
    uni_lower_matrix(std::size_t dim) noexcept : base_type(dim) {
        // Fill the diagonal
        for(size_t i = 0; i < etl::dim<0>(matrix); ++i){
            matrix(i,i) = value_type(1);
        }
     ...
     * \param rhs The right-hand-side matrix
     */
    uni_lower_matrix(uni_lower_matrix&& rhs) = default;
     ...
     * \return a reference to the assigned matrix
     */
    uni_lower_matrix& operator=(uni_lower_matrix&& rhs) = default;

...
/*!
 * \file
 * \brief Contains the sub_matrix_4d implementation
 */
...
public:
    ...
    const_return_type operator[](size_t f) const {
        ...
        if constexpr (storage_order == order::RowMajor) {
            ...
            return sub_expr(base_i + my_i, base_j + my_j, base_k + my_k, base_l + my_l);
        } else {
            // Extract 4D indices from flat inside the view
            auto my_l = f / (m * n * o);
            auto t1   = f % (m * n * o);
     ...
     */
    ETL_STRONG_INLINE(const_return_type) operator()(size_t i, size_t j, size_t k, size_t l) const {
     ...
     * \return a reference to the element at the given position.
     */
    ETL_STRONG_INLINE(return_type) operator()(size_t i, size_t j, size_t k, size_t l) {
        cpp_assert(i < m, "Invalid 4d index inside sub_matrix_4d");
        cpp_assert(j < n, "Invalid 4d index inside sub_matrix_4d");
        cpp_assert(k < o, "Invalid 4d index inside sub_matrix_4d");
        cpp_assert(l < p, "Invalid 4d index inside sub_matrix_4d");
...
/*!
 * \brief Specialization for sub_matrix_4d
 */
template <typename T, bool Aligned>
struct etl_traits<etl::sub_matrix_4d<T, Aligned>> {
    using expr_t     = etl::sub_matrix_4d<T, Aligned>;  ///< The expression type

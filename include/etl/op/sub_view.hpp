...
template <typename T>
struct sub_view :
    iterable<sub_view<T>, has_direct_access<T>::value>,
    inplace_assignable<sub_view<T>>
{
    static constexpr bool dma = has_direct_access<T>::value;
    ...
    using iterable_base_type = iterable<this_type, has_direct_access<T>::value>;
     ...
     */
    template <typename E, cpp_enable_if(dma && all_dma<E>::value)>
     ...
     * \return true if the two expressions aliases, false otherwise
     */
    template <typename E, cpp_disable_if(dma && all_dma<E>::value)>
    bool alias(const E& rhs) const noexcept {

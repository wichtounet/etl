...
auto reshape(E&& value) -> fast_matrix_view<detail::build_identity_type<E>, all_dma<E>, Dims...> {
    static_assert(is_etl_expr<E>, "etl::reshape can only be used on ETL expressions");
    cpp_assert(decay_traits<E>::is_generator || size(value) == mul_all<Dims...>, "Invalid size for reshape");

    return fast_matrix_view<detail::build_identity_type<E>, all_dma<E>, Dims...>{value};
}

 ...
 */
template <typename A, typename B, typename C, cpp_enable_if((all_row_major<A> && all_homogeneous<A, B, C>))>
 ...
 * \param c The result vector
 */
template <typename A, typename B, typename C, cpp_enable_if((all_column_major<A> && all_homogeneous<A, B, C>))>
void gemv(A&& a, B&& b, C&& c) {
    cpp_assert(vec_enabled, "At least one vector mode must be enabled for impl::VEC");

 ...
 */
template <typename A, typename B, typename C, cpp_enable_if(all_floating<A, B, C> && all_homogeneous<A, B, C>::value)>
 ...
 * \param c The output matrix
 */
template <typename A, typename B, typename C, cpp_disable_if(all_floating<A, B, C> && all_homogeneous<A, B, C>::value)>
void conv1_full_fft(A&& a, B&& b, C&& c) {
    cpp_unused(a);
    cpp_unused(b);
    cpp_unused(c);

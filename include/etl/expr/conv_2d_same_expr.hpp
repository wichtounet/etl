...
struct conv_2d_same_expr : base_temporary_expr_bin<conv_2d_same_expr<A, B, Flipped>, A, B> {
    ...
    void assign_to(C&& c)  const {
        static_assert(all_etl_expr<A, B, C>::value, "conv2_same only supported for ETL expressions");
...
conv_2d_same_expr<A, B, false> conv_2d_same(A&& a, B&& b) {
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return conv_2d_same_expr<A, B, false>{a, b};
}
 ...
 */
template <typename A, typename B>
conv_2d_same_expr<A, B, true> conv_2d_same_flipped(A&& a, B&& b) {
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return conv_2d_same_expr<A, B, true>{a, b};
}

...
struct batch_outer_product_expr : impl_expr<batch_outer_product_expr<T>, T> {
    ...
    static void apply(A&& a, B&& b, C&& c) {
        static_assert(all_etl_expr<A, B, C>::value, "Outer product only supported for ETL expressions");
        static_assert(decay_traits<A>::dimensions() == 2 && decay_traits<B>::dimensions() == 2 && decay_traits<C>::dimensions() == 2, "Invalid dimensions for outer product");

        detail::batch_outer_product_impl::apply(std::forward<A>(a), std::forward<B>(b), std::forward<C>(c));
    }

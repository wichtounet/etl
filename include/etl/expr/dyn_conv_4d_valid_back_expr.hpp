...
struct dyn_conv_4d_valid_back_expr : base_temporary_expr_bin<dyn_conv_4d_valid_back_expr<A, B, Flipped>, A, B> {
    ...
    using this_type   = dyn_conv_4d_valid_back_expr<A, B, Flipped>;    ///< The type of this expression
...
struct etl_traits<etl::dyn_conv_4d_valid_back_expr<A, B, Flipped>> {
    using expr_t       = etl::dyn_conv_4d_valid_back_expr<A, B, Flipped>; ///< The expression type
    ...
    static size_t size(const expr_t& e) {
        return etl::dim(e._a, 0) * etl::dim(e._b, 1) * ((etl::dim(e._a, 2) - etl::dim(e._b, 2) + 2 * e.p1) / e.s1 + 1) * ((etl::dim(e._a, 3) - etl::dim(e._b, 3) + 2 * e.p2) / e.s2 + 1);
    }
...
dyn_conv_4d_valid_back_expr<A, B, false> conv_4d_valid_back(A&& a, B&& b, size_t s1, size_t s2, size_t p1 = 0, size_t p2 = 0){
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return dyn_conv_4d_valid_back_expr<A, B, false>{a, b, s1, s2, p1, p2};
}
...
/*!
 * \brief Creates an expression representing the valid_back 4d convolution of a and b
 * \param a The input expression
 * \param b The kernel expression
 * \param s1 The first dimension stride
 * \param s2 The second dimension stride
 ...
 */
template <typename A, typename B>
dyn_conv_4d_valid_back_expr<A, B, true> conv_4d_valid_back_flipped(A&& a, B&& b, size_t s1, size_t s2, size_t p1 = 0, size_t p2 = 0){
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return dyn_conv_4d_valid_back_expr<A, B, true>{a, b, s1, s2, p1, p2};
}

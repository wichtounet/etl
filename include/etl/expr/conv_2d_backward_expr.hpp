...
struct conv_2d_backward_expr : base_temporary_expr_bin<conv_2d_backward_expr<A, B, S1, S2, P1, P2, Flipped>, A, B> {
    ...
    void assign_to(C&& c)  const {
        ...
        if /* constexpr */ (Flipped){
            detail::conv2_backward_flipped_impl<S1, S2, P1, P2>::apply(make_temporary(a), make_temporary(b), c);
        } else {
            detail::conv2_backward_impl<S1, S2, P1, P2>::apply(make_temporary(a), make_temporary(b), c);
        }
    }
...
struct etl_traits<etl::conv_2d_backward_expr<A, B, S1, S2, P1, P2, Flipped>> {
    ...
    static constexpr size_t dim() {
        return DD == 0 ? (S1 * (etl::dim<0, A>() - 1) + etl::dim<0, B>() - 2 * P1)
                       : (S2 * (etl::dim<1, A>() - 1) + etl::dim<1, B>() - 2 * P2);
    }
    ...
    static size_t size(const expr_t& e) {
        return (S1 * (etl::dim(e._a, 0) - 1) + etl::dim(e._b, 0) - 2 * P1) *
               (S2 * (etl::dim(e._a, 1) - 1) + etl::dim(e._b, 1) - 2 * P2);
    }
    ...
    static constexpr size_t size() {
        return   (S1 * (etl::dim<0, A>() - 1) + etl::dim<0, B>() - 2 * P1)
               * (S2 * (etl::dim<1, A>() - 1) + etl::dim<1, B>() - 2 * P2);
    }

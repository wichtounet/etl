...
struct conv_4d_valid_expr : base_temporary_expr_bin<conv_4d_valid_expr<A, B, S1, S2, P1, P2, Flipped>, A, B> {
    ...
    using this_type   = conv_4d_valid_expr<A, B, S1, S2, P1, P2, Flipped>;         ///< The type of this expression
    ...
    void assign_to(C&& c)  const {
        static_assert(all_etl_expr<A, B, C>::value, "conv4_valid only supported for ETL expressions");
        ...
        if /* constexpr */ (Flipped){
            detail::conv4_valid_flipped_impl<S1, S2, P1, P2>::apply(make_temporary(a), make_temporary(b), c);
        } else {
            detail::conv4_valid_impl<S1, S2, P1, P2>::apply(make_temporary(a), make_temporary(b), c);
        }
    }
...
struct etl_traits<etl::conv_4d_valid_expr<A, B, S1, S2, P1, P2, Flipped>> {
    using expr_t       = etl::conv_4d_valid_expr<A, B, S1, S2, P1, P2, Flipped>; ///< The expression type
    ...
    static size_t size(const expr_t& e) {
        return etl::dim(e._a, 0) * etl::dim(e._b, 0) * ((etl::dim(e._a, 2) - etl::dim(e._b, 2) + 2 * P1) / S1 + 1) * ((etl::dim(e._a, 3) - etl::dim(e._b, 3) + 2 * P2) / S2 + 1);
    }
    ...
    static constexpr size_t size() {
        return etl::dim<0, A>() * etl::dim<0, B>() * ((etl::dim<2, A>() - etl::dim<2, B>() + 2 * P1) / S1 + 1) * ((etl::dim<3, A>() - etl::dim<3, B>() + 2 * P2) / S2 + 1);
    }
...
conv_4d_valid_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, false> conv_4d_valid(A&& a, B&& b) {
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return conv_4d_valid_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, false>{a, b};
}
 ...
 */
template <size_t S1 = 1, size_t S2 = 1, size_t P1 = 0, size_t P2 = 0, typename A, typename B>
conv_4d_valid_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, true> conv_4d_valid_flipped(A&& a, B&& b) {
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return conv_4d_valid_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, true>{a, b};
}

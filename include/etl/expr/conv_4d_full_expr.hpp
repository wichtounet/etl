...
struct conv_4d_full_expr : base_temporary_expr_bin<conv_4d_full_expr<A, B, Flipped>, A, B> {
    ...
    void assign_to(C&& c)  const {
        static_assert(all_etl_expr<A, B, C>::value, "conv4_full only supported for ETL expressions");
        ...
        if /* constexpr */ (Flipped){
            ...
            detail::conv4_full_impl::apply(make_temporary(a), make_temporary(b), c);
        }
    }
...
struct etl_traits<etl::conv_4d_full_expr<A, B, Flipped>> {
    ...
    static constexpr size_t dim() {
        return DD == 0 ? etl::dim<DD, A>()
            :  DD == 1 ? etl::dim<DD, B>()
                       : etl::dim<DD, A>() + etl::dim<DD, B>() - 1;
    }

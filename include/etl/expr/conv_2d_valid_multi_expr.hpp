...
struct conv_2d_valid_multi_expr : base_temporary_expr_bin<conv_2d_valid_multi_expr<A, B, S1, S2, P1, P2, Flipped>, A, B> {
     ...
     */
    explicit conv_2d_valid_multi_expr(A a, B b) : base_type(a, b) {
    ...
    static void check(const I& input, const K& kernel, const C& conv){
        ...
        cpp_assert(etl::dim(conv, 2) == (etl::dim(input, 1) - etl::dim(kernel, 2) + 2 * P2) / S2 + 1, "Invalid dimensions for conv2_valid");
    ...
    static void check(const I& input, const K& kernel, const C& conv){
        ...
        static_assert(etl::dim<1, C>() == (etl::dim<0, I>() - etl::dim<1, K>() + 2 * P1) / S1 + 1, "Invalid dimensions for conv2_valid");
        static_assert(etl::dim<2, C>() == (etl::dim<1, I>() - etl::dim<2, K>() + 2 * P2) / S2 + 1, "Invalid dimensions for conv2_valid");
    ...
    void assign_to(C&& c)  const {
        ...
        if /* constexpr */ (Flipped){
            detail::conv2_valid_multi_flipped_impl<S1, S2, P1, P2>::apply(make_temporary(a), make_temporary(b), c);
        } else {
            detail::conv2_valid_multi_impl<S1, S2, P1, P2>::apply(make_temporary(a), make_temporary(b), c);
        }
    }
...
conv_2d_valid_multi_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, false> conv_2d_valid_multi(A&& a, B&& b) {
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return conv_2d_valid_multi_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, false>{a, b};
}
 ...
 */
template <size_t S1 = 1, size_t S2 = 1, size_t P1 = 0, size_t P2 = 0, typename A, typename B>
conv_2d_valid_multi_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, true> conv_2d_valid_multi_flipped(A&& a, B&& b) {
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return conv_2d_valid_multi_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, true>{a, b};
}

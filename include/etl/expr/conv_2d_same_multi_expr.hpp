...
struct conv_2d_same_multi_expr : base_temporary_expr_bin<conv_2d_same_multi_expr<A, B, Flipped>, A, B> {
    ...
    void assign_to(C&& c)  const {
        static_assert(all_etl_expr<A, B, C>::value, "conv2_same_multi only supported for ETL expressions");
        ...
        if /* constexpr */ (Flipped){
            detail::conv2_same_multi_flipped_impl::apply(make_temporary(a), make_temporary(b), c);
        } else {
            detail::conv2_same_multi_impl::apply(make_temporary(a), make_temporary(b), c);
        }
    }
...
conv_2d_same_multi_expr<A, B, false> conv_2d_same_multi(A&& a, B&& b) {
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return conv_2d_same_multi_expr<A, B, false>{a, b};
}
 ...
 */
template <typename A, typename B>
conv_2d_same_multi_expr<A, B, true> conv_2d_same_multi_flipped(A&& a, B&& b) {
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return conv_2d_same_multi_expr<A, B, true>{a, b};
}

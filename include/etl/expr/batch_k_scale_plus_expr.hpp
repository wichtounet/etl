...
struct batch_k_scale_plus_expr : base_temporary_expr_tern<batch_k_scale_plus_expr<A, B, C>, A, B, C> {
    ...
    static constexpr bool gpu_computable =
               (!D4 && impl::egblas::has_sbatch_k_scale_plus2 && all_row_major<A, B> && all_floating<A, B>)
            || (D4  && impl::egblas::has_dbatch_k_scale_plus4 && all_row_major<A, B> && all_floating<A, B>);

    /*!
     * \brief Construct a new expression
     * \param a The sub expression
     */
    batch_k_scale_plus_expr(A a, B b, C c) : base_type(a, b, c) {
    ...
    static void check([[maybe_unused]] const A& a, [[maybe_unused]] const B& b, [[maybe_unused]] const C& c, [[maybe_unused]] L& lhs) {
        if constexpr (D4) {
            ...
            static_assert(etl::dimensions<A>() == 1, "The lhs of batch_k_scale_plus is a 1D matrix");
            static_assert(etl::dimensions<B>() == 4, "The rhs of batch_k_scale_plus is a 4D matrix");
            static_assert(etl::dimensions<C>() == 1, "The beta of batch_k_scale_plus is a 1D matrix");

            if constexpr (all_fast<A, B, C, L>) {
                static_assert(etl::dim<0, B>() == etl::dim<0, L>(), "Invalid dimensions for batch_k_scale_plus");
                static_assert(etl::dim<1, B>() == etl::dim<1, L>(), "Invalid dimensions for batch_k_scale_plus");
                static_assert(etl::dim<2, B>() == etl::dim<2, L>(), "Invalid dimensions for batch_k_scale_plus");
                static_assert(etl::dim<3, B>() == etl::dim<3, L>(), "Invalid dimensions for batch_k_scale_plus");

                static_assert(etl::dim<0, A>() == etl::dim<1, B>(), "Invalid dimensions for batch_k_scale_plus");
                static_assert(etl::dim<0, A>() == etl::dim<0, C>(), "Invalid dimensions for batch_k_scale_plus");
            } else {
                cpp_assert(etl::dim<0>(b) == etl::dim<0>(lhs), "Invalid dimensions for batch_k_scale_plus");
                cpp_assert(etl::dim<1>(b) == etl::dim<1>(lhs), "Invalid dimensions for batch_k_scale_plus");
                cpp_assert(etl::dim<2>(b) == etl::dim<2>(lhs), "Invalid dimensions for batch_k_scale_plus");
                cpp_assert(etl::dim<3>(b) == etl::dim<3>(lhs), "Invalid dimensions for batch_k_scale_plus");

                cpp_assert(etl::dim<0>(a) == etl::dim<1>(b), "Invalid dimensions for batch_k_scale_plus");
                cpp_assert(etl::dim<0>(a) == etl::dim<0>(c), "Invalid dimensions for batch_k_scale_plus");
            }
        } else {
            static_assert(etl::dimensions<L>() == 2, "The output of batch_k_scale_plus is a 2D matrix");
            static_assert(etl::dimensions<A>() == 1, "The lhs of batch_k_scale_plus is a 1D matrix");
            static_assert(etl::dimensions<B>() == 2, "The rhs of batch_k_scale_plus is a 2D matrix");
            static_assert(etl::dimensions<C>() == 1, "The beta of batch_k_scale_plus is a 1D matrix");
    ...
    void assign_to(L&& lhs) const {
        ...
        if constexpr (D4) {
            ...
            if constexpr (impl::egblas::has_sbatch_k_scale4 && all_row_major<A, B, L> && all_floating<A, B, L>) {
                ...
                impl::egblas::batch_k_scale_plus(Batch, K, M, N, t2.gpu_memory(), t1.gpu_memory(), t3.gpu_memory(), lhs.gpu_memory());
                            ...
                            for (size_t batch = first; batch < last; ++batch) {
                                for (size_t k = 0; k < K; ++k) {
                                    ...
                                    for (; mn + 4 * vec_size - 1 < MN; mn += 4 * vec_size) {
                                        ...
                                        auto r1 = vec_type::add(vec_type::mul(a1, b1), c1);
                                        auto r2 = vec_type::add(vec_type::mul(a1, b2), c1);
                                        auto r3 = vec_type::add(vec_type::mul(a1, b3), c1);
                                        auto r4 = vec_type::add(vec_type::mul(a1, b4), c1);
            ...
            if constexpr (impl::egblas::has_sbatch_k_scale_plus2 && all_row_major<A, B, L> && all_floating<A, B, L>) {
                decltype(auto) t1 = smart_forward_gpu(a);
                decltype(auto) t2 = smart_forward_gpu(b);
                decltype(auto) t3 = smart_forward_gpu(c);
                ...
                impl::egblas::batch_k_scale_plus(Batch, K, t2.gpu_memory(), t1.gpu_memory(), t3.gpu_memory(), lhs.gpu_memory());
                        ...
                        if constexpr (vec_enabled && all_vectorizable<vector_mode, A, B, L> && all_row_major<A, B, L>) {
                            ...
                            for (size_t batch = first; batch < last; ++batch) {
                                ...
                                for (; k + 4 * vec_size - 1 < K; k += 4 * vec_size) {
                                    ...
                                    auto r1 = vec_type::add(vec_type::mul(a1, b1), c1);
                                    auto r2 = vec_type::add(vec_type::mul(a2, b2), c2);
                                    auto r3 = vec_type::add(vec_type::mul(a3, b3), c3);
                                    auto r4 = vec_type::add(vec_type::mul(a4, b4), c4);
    ...
    void assign_add_to(L&& lhs) const {
        ...
        if constexpr (D4) {
            if constexpr (impl::egblas::has_sbatch_k_scale_plus4 && all_row_major<A, B, L> && all_floating<A, B, L>) {
                            ...
                            for (size_t batch = first; batch < last; ++batch) {
                                for (size_t k = 0; k < K; ++k) {
                                    ...
                                    for (; mn + 4 * vec_size - 1 < MN; mn += 4 * vec_size) {
                                        ...
                                        auto r1 = vec_type::add(l1, vec_type::add(vec_type::mul(a1, b1), c1));
                                        auto r2 = vec_type::add(l2, vec_type::add(vec_type::mul(a1, b2), c1));
                                        auto r3 = vec_type::add(l3, vec_type::add(vec_type::mul(a1, b3), c1));
                                        auto r4 = vec_type::add(l4, vec_type::add(vec_type::mul(a1, b4), c1));
                        ...
                        if constexpr (vec_enabled && all_vectorizable<vector_mode, A, B, L> && all_row_major<A, B, L>) {
                            ...
                            for (size_t batch = first; batch < last; ++batch) {
                                ...
                                for (; k + 4 * vec_size - 1 < K; k += 4 * vec_size) {
                                    ...
                                    auto r1 = vec_type::add(l1, vec_type::add(vec_type::mul(a1, b1), c1));
                                    auto r2 = vec_type::add(l2, vec_type::add(vec_type::mul(a2, b2), c2));
                                    auto r3 = vec_type::add(l3, vec_type::add(vec_type::mul(a3, b3), c3));
                                    auto r4 = vec_type::add(l4, vec_type::add(vec_type::mul(a4, b4), c4));
                            ...
                            for (size_t batch = first; batch < last; ++batch) {
                                for (size_t k = 0; k < K; ++k) {
                                    ...
                                    for (; mn + 4 * vec_size - 1 < MN; mn += 4 * vec_size) {
                                        ...
                                        auto r1 = vec_type::sub(l1, vec_type::add(vec_type::mul(a1, b1), c1));
                                        auto r2 = vec_type::sub(l2, vec_type::add(vec_type::mul(a1, b2), c1));
                                        auto r3 = vec_type::sub(l3, vec_type::add(vec_type::mul(a1, b3), c1));
                                        auto r4 = vec_type::sub(l4, vec_type::add(vec_type::mul(a1, b4), c1));
                        ...
                        if constexpr (vec_enabled && all_vectorizable<vector_mode, A, B, L> && all_row_major<A, B, L>) {
                            ...
                            for (size_t batch = first; batch < last; ++batch) {
                                ...
                                for (; k + 4 * vec_size - 1 < K; k += 4 * vec_size) {
                                    ...
                                    auto r1 = vec_type::sub(l1, vec_type::add(vec_type::mul(a1, b1), c1));
                                    auto r2 = vec_type::sub(l2, vec_type::add(vec_type::mul(a2, b2), c2));
                                    auto r3 = vec_type::sub(l3, vec_type::add(vec_type::mul(a3, b3), c3));
                                    auto r4 = vec_type::sub(l4, vec_type::add(vec_type::mul(a4, b4), c4));
                            ...
                            for (size_t batch = first; batch < last; ++batch) {
                                for (size_t k = 0; k < K; ++k) {
                                    ...
                                    for (; mn + 4 * vec_size - 1 < MN; mn += 4 * vec_size) {
                                        ...
                                        auto r1 = vec_type::mul(l1, vec_type::add(vec_type::mul(a1, b1), c1));
                                        auto r2 = vec_type::mul(l2, vec_type::add(vec_type::mul(a1, b2), c1));
                                        auto r3 = vec_type::mul(l3, vec_type::add(vec_type::mul(a1, b3), c1));
                                        auto r4 = vec_type::mul(l4, vec_type::add(vec_type::mul(a1, b4), c1));
                        ...
                        if constexpr (vec_enabled && all_vectorizable<vector_mode, A, B, L> && all_row_major<A, B, L>) {
                            ...
                            for (size_t batch = first; batch < last; ++batch) {
                                ...
                                for (; k + 4 * vec_size - 1 < K; k += 4 * vec_size) {
                                    ...
                                    auto r1 = vec_type::mul(l1, vec_type::add(vec_type::mul(a1, b1), c1));
                                    auto r2 = vec_type::mul(l2, vec_type::add(vec_type::mul(a2, b2), c2));
                                    auto r3 = vec_type::mul(l3, vec_type::add(vec_type::mul(a3, b3), c3));
                                    auto r4 = vec_type::mul(l4, vec_type::add(vec_type::mul(a4, b4), c4));
                            ...
                            for (size_t batch = first; batch < last; ++batch) {
                                for (size_t k = 0; k < K; ++k) {
                                    ...
                                    for (; mn + 4 * vec_size - 1 < MN; mn += 4 * vec_size) {
                                        ...
                                        auto r1 = vec_type::div(l1, vec_type::add(vec_type::mul(a1, b1), c1));
                                        auto r2 = vec_type::div(l2, vec_type::add(vec_type::mul(a1, b2), c1));
                                        auto r3 = vec_type::div(l3, vec_type::add(vec_type::mul(a1, b3), c1));
                                        auto r4 = vec_type::div(l4, vec_type::add(vec_type::mul(a1, b4), c1));
                        ...
                        if constexpr (vec_enabled && all_vectorizable<vector_mode, A, B, L> && all_row_major<A, B, L>) {
                            ...
                            for (size_t batch = first; batch < last; ++batch) {
                                ...
                                for (; k + 4 * vec_size - 1 < K; k += 4 * vec_size) {
                                    ...
                                    auto r1 = vec_type::div(l1, vec_type::add(vec_type::mul(a1, b1), c1));
                                    auto r2 = vec_type::div(l2, vec_type::add(vec_type::mul(a2, b2), c2));
                                    auto r3 = vec_type::div(l3, vec_type::add(vec_type::mul(a3, b3), c3));
                                    auto r4 = vec_type::div(l4, vec_type::add(vec_type::mul(a4, b4), c4));
    ...
    friend std::ostream& operator<<(std::ostream& os, const batch_k_scale_plus_expr& expr) {
        return os << "batch_k_scale_plus(" << expr._a << "," << expr._b << "," << expr._c << ")";
    }
};
...
struct etl_traits<etl::batch_k_scale_plus_expr<A, B, C>> {
    using expr_t     = etl::batch_k_scale_plus_expr<A, B, C>; ///< The expression type

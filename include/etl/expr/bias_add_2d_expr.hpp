...
struct bias_add_2d_expr : base_temporary_expr_bin<bias_add_2d_expr<A, B>, A, B> {
    ...
    void assign_to(L&& lhs)  const {
        static_assert(all_etl_expr<A, L>::value, "bias_add_2d only supported for ETL expressions");
        ...
        if(impl == bias_add_impl::VEC){
            ...
            impl::standard::bias_add_2d(a, make_temporary(b), lhs);
        } else if(impl == bias_add_impl::CUDNN){
            impl::cudnn::bias_add_2d(a, make_temporary(b), lhs);
        } else {
...
bias_add_2d_expr<detail::build_type<E>, detail::build_type<B>> bias_add_2d(const E& x, const B& biases){
    static_assert(all_etl_expr<E, B>::value, "etl::bias_add_2d can only be used on ETL expressions");

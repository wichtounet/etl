...
struct etl_traits<etl::conv_2d_valid_multi_multi_expr<A, B, S1, S2, P1, P2, Flipped>> {
    ...
    static constexpr size_t dim() {
        return DD == 0 ? etl::dim<0, B>()
                       : DD == 1 ? etl::dim<0, A>()
                                 : DD == 2 ? (etl::dim<1, A>() - etl::dim<1, B>() + 2 * P1) / S1 + 1 : (etl::dim<2, A>() - etl::dim<2, B>() + 2 * P2) / S2 + 1;
    }
    ...
    static size_t size(const expr_t& e) {
        return (etl::dim(e._b, 0)) * (etl::dim(e._a, 1)) * ((etl::dim(e._a, 1) - etl::dim(e._b, 1) + 2 * P1) / S1 + 1)
    ...
    static constexpr size_t size() {
        return (etl::dim<0, B>()) * (etl::dim<0, A>()) * ((etl::dim<1, A>() - etl::dim<1, B>() + 2 * P1) / S1 + 1)
...
conv_2d_valid_multi_multi_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P2, false> conv_2d_valid_multi_multi(A&& a, B&& b) {
    ...
    return conv_2d_valid_multi_multi_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P2, false>{a, b};
}
...
conv_2d_valid_multi_multi_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P2, true> conv_2d_valid_multi_multi_flipped(A&& a, B&& b) {
    ...
    return conv_2d_valid_multi_multi_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P2, true>{a, b};
}

...
struct conv_2d_valid_multi_multi_expr : base_temporary_expr_bin<conv_2d_valid_multi_multi_expr<A, B, S1, S2, P1, P2, Flipped>, A, B> {
    using value_type  = value_t<A>;                                              ///< The type of value of the expression
    using this_type   = conv_2d_valid_multi_multi_expr<A, B, S1, S2, P1, P2, Flipped>; ///< The type of this expression
     ...
     */
    explicit conv_2d_valid_multi_multi_expr(A a, B b) : base_type(a, b) {
    ...
    static void check(const I& input, const K& kernel, const C& conv){
        static_assert(etl::dimensions<I>() == 3, "Invalid number of dimensions for input of conv2_valid");
        static_assert(etl::dimensions<K>() == 3, "Invalid number of dimensions for kernel of conv2_valid");
        static_assert(etl::dimensions<C>() == 4, "Invalid number of dimensions for conv of conv2_valid");

        cpp_assert(etl::dim(conv, 0) == etl::dim(kernel, 0), "Invalid dimensions for conv2_valid");
        cpp_assert(etl::dim(conv, 1) == etl::dim(input, 0), "Invalid dimensions for conv2_valid");
        cpp_assert(etl::dim(conv, 2) == (etl::dim(input, 1) - etl::dim(kernel, 1) + 2 * P1) / S1 + 1, "Invalid dimensions for conv2_valid");
        cpp_assert(etl::dim(conv, 3) == (etl::dim(input, 2) - etl::dim(kernel, 2) + 2 * P2) / S2 + 1, "Invalid dimensions for conv2_valid");
    ...
    static void check(const I& input, const K& kernel, const C& conv){
        ...
        static_assert(etl::dim<0, C>() == etl::dim<0, K>(), "Invalid dimensions for conv2_valid");
        static_assert(etl::dim<1, C>() == etl::dim<0, I>(), "Invalid dimensions for conv2_valid");
        static_assert(etl::dim<2, C>() == (etl::dim<1, I>() - etl::dim<1, K>() + 2 * P1) / S1 + 1, "Invalid dimensions for conv2_valid");
        static_assert(etl::dim<3, C>() == (etl::dim<2, I>() - etl::dim<2, K>() + 2 * P2) / S2 + 1, "Invalid dimensions for conv2_valid");
    ...
    /*!
     * \brief multi_multiply the given left-hand-side expression
     * \param lhs The expression to which assign
     */
    template<typename L>
    ...
    friend std::ostream& operator<<(std::ostream& os, const conv_2d_valid_multi_multi_expr& expr) {
        return os << "conv2_valid_multi_multi(" << expr._a << ", " << expr._b << ")";
    }
};
...
struct etl_traits<etl::conv_2d_valid_multi_multi_expr<A, B, S1, S2, P1, P2, Flipped>> {
    using expr_t       = etl::conv_2d_valid_multi_multi_expr<A, B, S1, S2, P1, P2, Flipped>; ///< The expression type
    ...
    static constexpr size_t dim() {
        return DD == 0 ? etl::dim<0, B>()
             : DD == 1 ? etl::dim<0, A>()
             : DD == 2 ? (etl::dim<1, A>() - etl::dim<1, B>() + 2 * P1) / S1 + 1
                       : (etl::dim<2, A>() - etl::dim<2, B>() + 2 * P2) / S2 + 1;
    }
    ...
    static size_t dim(const expr_t& e, size_t d) {
        if (d == 0){
            ...
            return (etl::dim(e._a, 1) - etl::dim(e._b, 1) + 2 * P1) / S1 + 1;
        } else {
            return (etl::dim(e._a, 2) - etl::dim(e._b, 2) + 2 * P2) / S2 + 1;
        }
    }
    ...
    static size_t size(const expr_t& e) {
        return (etl::dim(e._b, 0))
            *  (etl::dim(e._a, 1))
            *  ((etl::dim(e._a, 1) - etl::dim(e._b, 1) + 2 * P1) / S1 + 1)
            *  ((etl::dim(e._a, 2) - etl::dim(e._b, 2) + 2 * P2) / S2 + 1);
    }
    ...
    static constexpr size_t size() {
        return  (etl::dim<0, B>())
              * (etl::dim<0, A>())
              * ((etl::dim<1, A>() - etl::dim<1, B>() + 2 * P1) / S1 + 1)
              * ((etl::dim<2, A>() - etl::dim<2, B>() + 2 * P2) / S2 + 1);
    }
...
conv_2d_valid_multi_multi_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, false> conv_2d_valid_multi_multi(A&& a, B&& b) {
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return conv_2d_valid_multi_multi_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, false>{a, b};
}
...
auto conv_2d_valid_multi_multi(A&& a, B&& b, C&& c){
    ...
    c = conv_2d_valid_multi_multi<S1, S2, P1, P2>(a, b);

    return c;
}

/*!
 * \brief Creates an expression representing the 'valid' 2D convolution of a and flipped b.
 *
 * \param a The input expression
 * \param b The kernel expression
 *
 ...
 */
template <size_t S1 = 1, size_t S2 = 1, size_t P1 = 0, size_t P2 = 0, typename A, typename B>
conv_2d_valid_multi_multi_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, true> conv_2d_valid_multi_multi_flipped(A&& a, B&& b) {
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return conv_2d_valid_multi_multi_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, true>{a, b};
}

/*!
 * \brief Creates an expression representing the 'valid' 2D convolution of a and flipped b, the result will be stored in c
 *
 * \param a The input expression
 * \param b The kernel expression
 * \param c The result
...
auto conv_2d_valid_multi_multi_flipped(A&& a, B&& b, C&& c){
    ...
    c = conv_2d_valid_multi_multi_flipped<S1, S2, P1, P2>(a, b);

...
public:
     ...
     */
    dyn_pool_upsample_2d_expr(A a, B b, C c, size_t c1, size_t c2)
            : base_type(a, b, c), c1(c1), c2(c2) {
    ...
    void assign_to(R&& result) const {
        ...
        if constexpr (Max) {
            ...
            if constexpr_select (impl == pool_impl::STD) {
                impl::standard::avg_pool_upsample_2d::apply(
                    smart_forward(a),
                    smart_forward(b),
                    smart_forward(c),
                    result,
...
avg_pool_upsample_2d(A&& input, B&& output, C&& errors, size_t c1, size_t c2) {
    return {input, output, errors, c1, c2};
}

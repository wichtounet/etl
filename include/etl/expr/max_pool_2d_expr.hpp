...
struct max_pool_2d_expr : base_temporary_expr_un<max_pool_2d_expr<A, C1, C2, S1, S2, P1, P2>, A> {
    using value_type = value_t<A>;                           ///< The type of value of the expression
    using this_type  = max_pool_2d_expr<A, C1, C2, S1, S2, P1, P2>;                    ///< The type of this expression
     ...
     */
    explicit max_pool_2d_expr(A a) : base_type(a) {
 ...
 */
template <typename A, size_t C1, size_t C2, size_t S1, size_t S2, size_t P1, size_t P2>
struct etl_traits<etl::max_pool_2d_expr<A, C1, C2, S1, S2, P1, P2>> {
    using expr_t     = etl::max_pool_2d_expr<A, C1, C2, S1, S2, P1, P2>; ///< The expression type
    ...
    static std::size_t dim(const expr_t& e, std::size_t d) {
        if (d == D - 2) {
            return (etl::dim(e.a_, d) - C1 + 2 * P1) / S1 + 1;
        } else if (d == D - 1){
            return (etl::dim<1>(e.a_, d) - C2 + 2 * P2) / S2 + 1;
        } else {
            return etl::dim(e.a_, d);
        }
    }

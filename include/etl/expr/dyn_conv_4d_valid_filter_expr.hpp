...
struct dyn_conv_4d_valid_filter_expr : base_temporary_expr_bin<dyn_conv_4d_valid_filter_expr<A, B, Flipped>, A, B> {
    ...
    void assign_to(C&& c) const {
        static_assert(all_etl_expr<A, B, C>::value, "conv4_valid_filter only supported for ETL expressions");
        ...
        if /* constexpr */ (Flipped){
            detail::dyn_conv4_valid_filter_flipped_impl::apply(make_temporary(a), make_temporary(b), c, s1, s2, p1, p2);
        } else {
            detail::dyn_conv4_valid_filter_impl::apply(make_temporary(a), make_temporary(b), c, s1, s2, p1, p2);
        }
    }
...
struct etl_traits<etl::dyn_conv_4d_valid_filter_expr<A, B, Flipped>> {
    ...
    static size_t size(const expr_t& e) {
        return etl::dim(e._b, 1) * etl::dim(e._a, 1) * ((etl::dim(e._a, 2) - etl::dim(e._b, 2) + 2 * e.p1) / e.s1 + 1) * ((etl::dim(e._a, 3) - etl::dim(e._b, 3) + 2 * e.p2) / e.s2 + 1);
    }
...
dyn_conv_4d_valid_filter_expr<A, B, false> conv_4d_valid_filter(A&& a, B&& b, size_t s1, size_t s2, size_t p1 = 0, size_t p2 = 0){
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return dyn_conv_4d_valid_filter_expr<A, B, false>{a, b, s1, s2, p1, p2};
}
 ...
 */
template <typename A, typename B>
dyn_conv_4d_valid_filter_expr<A, B, true> conv_4d_valid_filter_flipped(A&& a, B&& b, size_t s1, size_t s2, size_t p1 = 0, size_t p2 = 0){
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return dyn_conv_4d_valid_filter_expr<A, B, true>{a, b, s1, s2, p1, p2};
}

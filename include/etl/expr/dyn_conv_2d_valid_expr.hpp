...
struct dyn_conv_2d_valid_expr : base_temporary_expr_bin<dyn_conv_2d_valid_expr<A, B, Flipped>, A, B> {
    ...
    void assign_to(C&& c) const {
        ...
        if constexpr (Flipped) {
            detail::dyn_conv2_valid_flipped_impl::apply(a, b, c, s1, s2, p1, p2);
        } else {
            detail::dyn_conv2_valid_impl::apply(a, b, c, s1, s2, p1, p2);
        }
    }
...
struct etl_traits<etl::dyn_conv_2d_valid_expr<A, B, Flipped>> {
    ...
    static size_t size(const expr_t& e) {
        return ((etl::dim(e._a, 0) - etl::dim(e._b, 0) + 2 * e.p1) / e.s1 + 1) * ((etl::dim(e._a, 1) - etl::dim(e._b, 1) + 2 * e.p2) / e.s2 + 1);
    }
 ...
 */
template <typename A, typename B>
dyn_conv_2d_valid_expr<detail::build_type<A>, detail::build_type<B>, true> conv_2d_valid_flipped(

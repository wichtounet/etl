...
struct etl_traits<etl::conv_4d_valid_filter_expr<A, B, S1, S2, P1, P2, Flipped>> {
    ...
    static constexpr size_t size() {
        return etl::dim<1, B>() * etl::dim<1, A>() * ((etl::dim<2, A>() - etl::dim<2, B>() + 2 * P1) / S1 + 1) * ((etl::dim<3, A>() - etl::dim<3, B>() + 2 * P2) / S2 + 1);
    }
...
conv_4d_valid_filter_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, false> conv_4d_valid_filter(A&& a, B&& b) {
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return conv_4d_valid_filter_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, false>{a, b};
}
 ...
 */
template <size_t S1 = 1, size_t S2 = 1, size_t P1 = 0, size_t P2 = 0, typename A, typename B>
conv_4d_valid_filter_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, true> conv_4d_valid_filter_flipped(A&& a, B&& b) {
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return conv_4d_valid_filter_expr<detail::build_type<A>, detail::build_type<B>, S1, S2, P1, P1, true>{a, b};
}

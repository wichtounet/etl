...
struct dyn_conv_4d_valid_expr : base_temporary_expr_bin<dyn_conv_4d_valid_expr<A, B, Flipped>, A, B> {
     ...
     */
    explicit dyn_conv_4d_valid_expr(A a, B b, size_t s1, size_t s2, size_t p1, size_t p2) : base_type(a, b), s1(s1), s2(s2), p1(p1), p2(p2) {
    ...
    void assign_to(C&& c) const {
        ...
        if /* constexpr */ (Flipped){
            detail::dyn_conv4_valid_flipped_impl::apply(make_temporary(a), make_temporary(b), c, s1, s2, p1, p2);
        } else {
            detail::dyn_conv4_valid_impl::apply(make_temporary(a), make_temporary(b), c, s1, s2, p1, p2);
        }
    }
     ...
     */
    friend std::ostream& operator<<(std::ostream& os, const dyn_conv_4d_valid_expr& expr) {
...
struct etl_traits<etl::dyn_conv_4d_valid_expr<A, B, Flipped>> {
    ...
    static size_t size(const expr_t& e) {
        return etl::dim(e._a, 0) * etl::dim(e._b, 0) * ((etl::dim(e._a, 2) - etl::dim(e._b, 2) + 2 * e.p1) / e.s1 + 1) * ((etl::dim(e._a, 3) - etl::dim(e._b, 3) + 2 * e.p2) / e.s2 + 1);
    }
...
dyn_conv_4d_valid_expr<A, B, false> conv_4d_valid(A&& a, B&& b, size_t s1, size_t s2, size_t p1 = 0, size_t p2 = 0){
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return dyn_conv_4d_valid_expr<A, B, false>{a, b, s1, s2, p1, p2};
}
 ...
 */
template <typename A, typename B>
dyn_conv_4d_valid_expr<A, B, true> conv_4d_valid_flipped(A&& a, B&& b, size_t s1, size_t s2, size_t p1 = 0, size_t p2 = 0){
    static_assert(all_etl_expr<A, B>::value, "Convolution only supported for ETL expressions");

    return dyn_conv_4d_valid_expr<A, B, true>{a, b, s1, s2, p1, p2};
}

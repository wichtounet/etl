...
template <typename E, typename I, typename... Subs>
struct fast_result_type_builder_alpha;

template <typename E, std::size_t... I, typename... Subs>
struct fast_result_type_builder_alpha<E, std::index_sequence<I...>, Subs...> {
    using value_type = typename decay_traits<E>::value_type;
    using type       = fast_matrix_impl<value_type, std::vector<value_type>, decay_traits<E>::storage_order, decay_traits<E>::template dim<I>()...>;
};

template <typename E, bool Fast, typename... Subs>
struct expr_result_alpha;

template <typename E, typename... Subs>
struct expr_result_alpha<E, false, Subs...> {
    using value_type = typename decay_traits<E>::value_type;
    using type = dyn_matrix_impl<value_type, decay_traits<E>::storage_order, decay_traits<E>::dimensions()>;
};

template <typename E, typename... Subs>
struct expr_result_alpha<E, true, Subs...> {
    using type = typename fast_result_type_builder_alpha<E, std::make_index_sequence<decay_traits<E>::dimensions()>, Subs...>::type;
};

template <typename E, typename... Subs>
using expr_result_t_alpha = typename expr_result_alpha<E, all_fast<Subs...>::value, Subs...>::type;
...
template <typename D>
struct impl_expr_alpha {
    using derived_t = D;

    template <typename... Subs>
    using result_type = detail::expr_result_t_alpha<derived_t, Subs...>;

...
TEMPLATE_TEST_CASE_2("direct_access/traits", "has_direct_access", Z, double, float) {
    ...
    REQUIRE_DIRECT(etl::has_direct_access<expr_1>::value);
    REQUIRE_DIRECT(etl::has_direct_access<expr_2>::value);

    using expr_3 = decltype(a + a);
    using expr_4 = decltype(etl::abs(a));

    REQUIRE_DIRECT(!etl::has_direct_access<expr_3>::value);
    REQUIRE_DIRECT(!etl::has_direct_access<expr_4>::value);
    ...
    REQUIRE_DIRECT(etl::has_direct_access<expr_5>::value);
    REQUIRE_DIRECT(etl::has_direct_access<expr_6>::value);
    REQUIRE_DIRECT(!etl::has_direct_access<expr_7>::value);
    REQUIRE_DIRECT(etl::has_direct_access<expr_8>::value);
    REQUIRE_DIRECT(!etl::has_direct_access<expr_9>::value);
    REQUIRE_DIRECT(etl::has_direct_access<expr_10>::value);
}
